...
export function drawPot(random: () => number = Math.random): {
  pot: number;
  foundBlock: boolean;
} {
...
export function outcomeOf(row: Round): Outcome {
  if (row.bid === null) return { won: false, delta: 0, label: "—", tone: "dim" };
...
export interface Stats {
  /** The middle round. Shown ahead of the average because the average is
   * dragged upward by the rare block, and a bidder who reads only the average
   * concludes the going rate is free money. The gap between the two is the
   * whole risk of the thing. */
 ...
 * has to say out loud. */
export function backtest(bid: number, rounds: readonly Round[]): number {
  let total = 0;
 ...
 * and a spike. */
export function barHeights(values: readonly number[]): readonly number[] {
...
/** Newest first. */
export function openingRounds(): readonly Round[] {
